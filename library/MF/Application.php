...
class MF_Application{
	...
	public function _autoload($class){
		...
		if( !file_exists($filename) && count($parts) == 1 ){
			...
			elseif ( file_exists( APPLICATION_PATH."/models/triggers/".$class.'.php' ) )
				$filename = APPLICATION_PATH."/models/triggers/".$class.'.php';
			else
				$filename = APPLICATION_PATH."/delegates/".$class.'.php';
		}
		try{
